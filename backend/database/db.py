...
def init_db():
    ...
    try:
        if DB_TYPE == "postgres":
            import psycopg2
            cursor = conn.cursor()
            pg_schema = EMBEDDED_SCHEMA.replace("INSERT OR IGNORE INTO", "INSERT INTO")
            # Convert SQLite inserts with ON CONFLICT DO NOTHING
            pg_statements = [s.strip() for s in pg_schema.split(";") if s.strip()]
